...
def main():
    ...
    # Dataset
    train_ds = ToothDataset(args.data_path, 'train',
                            args.img_size, augment=True)
    val_ds = ToothDataset(args.data_path, 'val',
                          args.img_size, augment=False)

    # Weighted sampler
    if hasattr(train_ds, "sample_weights") and train_ds.sample_weights:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'training_curves_full.png'), dpi=200)
    plt.close()
